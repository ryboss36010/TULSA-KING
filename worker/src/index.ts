...
export default {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // GET /odds/:sport — odds for a sport
  ...
  // Cron handler — syncs games, settles bets, and backs up ledger
  async scheduled(event: ScheduledEvent, env: Env): Promise<void> {
    const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY);

    // Nightly backup (5am UTC cron)
    const hour = new Date().getUTCHours();
    if (hour === 5) {
      await backupLedger(supabase, env);
    }

    // Sync games and settle bets (every 5 minutes)
    await syncGames(supabase, env);
    await settleBets(supabase, env);
  },
};

async function syncGames(supabase: any, env: Env) {
    ...
    try {
      ...
      for (const event of events) {
        const gameData = {
          ...
          status:
            new Date(event.commence_time) <= new Date() ? "live" : "upcoming",
        ...
        // Get the game ID for market upserts
